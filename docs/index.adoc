...
IMPORTANT: *Object References and Immutability* - Objects stored in the cache are returned by reference, not by value. This means that if you modify a retrieved object, those modifications will affect the cached object itself. If immutability is required, you must deep clone the returned object. For simple objects, you can use `JSON.parse(JSON.stringify(obj))`. For more complex objects with functions or circular references, consider using a dedicated deep cloning library.
...
[source,js]
----
// Solution: Deep clone when immutability is needed
var obj = cache.get('myObject', function() {
    return { a: { b: true } };
});
...
NOTE: Cached objects are stored and retrieved by reference. Modifying an object after storing it, or modifying an object retrieved from the cache, will affect the cached value. Deep clone the object if immutability is required.
