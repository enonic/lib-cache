...
IMPORTANT: *Object References and Immutability* - Objects stored in the cache are returned by reference, not by value. This means that if you modify a retrieved object, those modifications could affect the cached object itself. It is best to deep clone the returned object after retrieval if the value must be changed. For simple objects, you can use `JSON.parse(JSON.stringify(obj))`. For more complex objects with functions or circular references, consider using a dedicated deep cloning library.

.Example: Reference behavior
[source,js]
----
// Problem: Modifying cached objects
var obj = cache.get('myObject', function() {
    return { a: { b: true } };
});
obj.a.b = false;  // This modifies the cached object!

var obj2 = cache.get('myObject', function() {
    return { a: { b: true } };
});
// obj2.a.b is now false (not the expected true)
----

.Example: Solution with deep cloning
[source,js]
----
// Solution: Deep clone after retrieval if the value must be changed
var obj = cache.get('myObject', function() {
    return { a: { b: true } };
});
var clonedObj = JSON.parse(JSON.stringify(obj));
clonedObj.a.b = false;  // Only modifies the clone

var obj2 = cache.get('myObject', function() {
    return { a: { b: true } };
});
// obj2.a.b is still true (cache is unaffected)
----
...
NOTE: Cached objects are stored and retrieved by reference. Modifying an object after storing it, or modifying an object retrieved from the cache, could affect the cached value. It is best to deep clone after retrieval if the value must be changed.
